...
### Remote privacy gate before every GitHub push (Phase 1, T89) — 2026-09-18

Contributed by Corey Tyhurst (huey-holdings-llc), ported from the visibility
probe in his plugin and reshaped to this project's destination model.

**What changed.** `_push_github` now starts with `_push_github_gate`
(`lib/destinations.sh`). Every origin push URL that names a GitHub repository
...
`test/destinations.test.sh` itself pushes to a local bare repository as origin
in two places, and every user with a Gitea, GitLab or local remote would stop
pushing on upgrade with no way to say "this one is fine". Blocking other hosts
needs the explicit trust record (item 2's second half) to land with it, so it
is left as the next unit, together with item 4's fingerprint-bound persisted
decision and its status fields. What IS covered from Phase 1: the anonymous
canonical probe with redirects refused (item 3), the complete push URL set
rather than the fetch URL alone (item 1), the gate living in `push` only
(item 5), and per-destination results with partial failure (item 6).

**Evidence.** Specs were written first and confirmed failing against `main`'s
`bin/` and `lib/` before any production code existed: 34 of the new
destination specs and 2 of the 3 new dependency specs failed there (the rest
pin behaviour that must not change, such as a non-GitHub origin being pushed
to unchanged, and pass on both). Two existing fixtures that push to a
github.com origin through a fake `git` gained a fake `curl`, since no spec may
reach the real network; the whole suite was also run with the container's
network disabled.

`./test/run.sh destinations` — **137 passed, 0 failed**.
`./test/run.sh deps` — **22 passed, 0 failed**.
Full suite — **1385 passed, 51 failed**, against **1340 passed, 51 failed** for
`main` in the same environment, with an identical failing set. That
environment is a network-less `archlinux:latest` container, not a live Omarchy
desktop: the 51 are the `systemd-analyze --user verify` specs (no user
manager), the headless quickshell probes, and the panel specs that depend on
them. The full suite has NOT yet been run on a real Omarchy session for this
change, and no Herdr review round has been held; both are still owed before
this is merged.

**Found along the way, not fixed here.** `collect` dies on a machine with no
foreign packages: `pacman -Qqem` exits 1 when the query matches nothing, and
